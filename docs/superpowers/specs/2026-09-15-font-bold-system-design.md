...
2. 對 `base_path` 先找其對應的普通名稱（註記部分的字型家族名），再查 `"<家族名> bold"` 是否存在；命中即回傳粗體檔路徑。
3. 若家族名對應失敗，再加一條檔名規則作為備援：在 base_path 同目錄找 `*bold*`、`*-b*`（如 `arialbd.ttf`、`msjhbd.ttc`）的檔案，命中且為 TrueType/OpenType 者回傳。
4. 兩階段皆無 → 回傳 `None`（表示走模擬粗體）。
...
共有 5 處建立 `TextInsertion`／`TextReplacement` 的位置，全部在尾端補上粗體旗標：

1. `commit_inline_text` — `TextInsertion(...)`。
2. `commit_inline_text` — `TextReplacement(...)`。
3. `preview_from_panel` — `TextInsertion(...)`。
4. `preview_from_panel` — `TextReplacement(...)`。
5. `apply_preview` — `TextReplacement(...)`。

粗體旗標來源為 `self.text_panel.bold.isChecked()`。
...
- 缺字警告列：`QLabel`（`minimumHeight` 預留），初始隱藏；`text.textChanged` 與選單／自訂檔變更時觸發檢查。檢查使用 `pymupdf.Font(fontfile=path).has_glyph()`；目前 text 為空或為一般字重載入失敗時不顯示警告。
...
- 模擬粗體為工程近似，品質略遜於真實粗體檔；對標楷體等手寫風格字型效果一般。
